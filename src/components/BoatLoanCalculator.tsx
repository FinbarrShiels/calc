...
interface BoatLoanCalculatorProps {
...
const BoatLoanCalculator: React.FC<BoatLoanCalculatorProps> = ({ calculator }) => {
  // Currency options
  ...
  const calculateLoan = () => {
    // Calculate sales tax
    const salesTax = (boatPrice * salesTaxRate) / 100;
    
    // Calculate total boat cost including tax
    const totalBoatCost = boatPrice + salesTax;
    
    // Calculate loan amount
    const calculatedLoanAmount = totalBoatCost - downPayment;
    ...
    // Add monthly insurance if included
    if (includeInsurance) {
      calculatedMonthlyPayment += insuranceAnnual / 12;
    }
    ...
    const calculatedTotalCost = downPayment + totalPayments;
  ...
  useEffect(() => {
    calculateLoan();
  }, [boatPrice, downPayment, loanTerm, interestRate, salesTaxRate, includeInsurance, insuranceAnnual]);
    ...
    <div className="bg-gray-800 text-white rounded-lg shadow-xl p-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-900 rounded-lg p-5 border border-gray-700">
          <h2 className="text-xl font-bold text-white mb-4">Boat Loan Calculator</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="boatPrice" className="block text-sm font-medium text-gray-300 mb-1">
                Boat Price
              </label>
            ...
            <input
                  id="boatPrice"
              type="tel"
                  value={boatPrice} {...numericInputProps}
          ...
          <input
            ...
            <input
                ...
                />
                <label htmlFor="includeInsurance" className="ml-2 block text-sm font-medium text-gray-300">
                  Include Boat Insurance
                </label>
        </div>
        
              {includeInsurance && (
                <div className="mt-2">
                  <label htmlFor="insuranceAnnual" className="block text-sm font-medium text-gray-300 mb-1">
            ...
            <input
                      id="insuranceAnnual"
              type="tel"
                      value={insuranceAnnual} {...numericInputProps}
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-gray-300">Boat Price:</span>
                      <span className="text-white">{formatCurrency(boatPrice)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-300">Sales Tax ({formatPercentage(salesTaxRate)}):</span>
                      <span className="text-white">{formatCurrency((boatPrice * salesTaxRate) / 100)}</span>
          </div>
                    <div className="flex justify-between">
                ...
                <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-gray-300">Monthly Principal & Interest:</span>
                      <span className="text-white">{formatCurrency(monthlyPayment - (includeInsurance ? insuranceAnnual / 12 : 0))}</span>
                    </div>
                    {includeInsurance && (
                      <div className="flex justify-between">
                        <span className="text-gray-300">Monthly Insurance:</span>
                        <span className="text-white">{formatCurrency(insuranceAnnual / 12)}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <span className="text-gray-300">Total Monthly Payment:</span>
                      <span className="text-white font-semibold">{formatCurrency(monthlyPayment)}</span>
                    </div>
                    <div className="flex justify-between">
          ...
          {viewType === 'table' && (
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 overflow-x-auto">
              ...
              <div className="mt-4 bg-gray-900 p-3 rounded-lg border border-gray-700">
                <h4 className="text-md font-medium text-blue-400 mb-1">Boat Loan Tips</h4>
                <p className="text-sm text-gray-300">
                  A larger down payment can significantly reduce your monthly payments and total interest paid. 
                  Consider shopping around for the best interest rates, as even a small difference can save you thousands over the life of the loan.
                </p>
              </div>
            </div>
          )}
